...
<main class="main-content">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mb-3">
        ...
        <div>
            <form class="d-flex gap-2" method="get" action="<?= BASE_URL ?>">
                <input type="hidden" name="action" value="customers-edit">
    ...
    <?php if (!$selectedBooking || !$editingCustomer): ?>
        <div class="card-like">
    ...
    <?php else: ?>
        ...
        <div class="card-like">
            <h4 class="mb-3">Cập nhật khách: <?= htmlspecialchars($editingCustomer['full_name']) ?></h4>
            <form method="post" action="<?= BASE_URL ?>?action=customers-update">
                <input type="hidden" name="id" value="<?= (int)$editingCustomer['id'] ?>">
                <input type="hidden" name="booking_id" value="<?= (int)$selectedBooking['id'] ?>">
                <div class="mb-3">
                    <label class="form-label">Họ và tên <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="full_name" value="<?= htmlspecialchars($editingCustomer['full_name'] ?? '') ?>" required>
                </div>
                <div class="row g-3">
                    <div class="col-12 col-md-6">
                        ...
                        <select class="form-select" name="gender">
                            ...
                            <?php foreach ($genderOptions as $value => $label): ?>
                                <option value="<?= $value ?>" <?= ($editingCustomer['gender'] ?? '') === $value ? 'selected' : '' ?>><?= $label ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-12 col-md-6">
                        <label class="form-label">Ngày sinh</label>
                        <input type="date" class="form-control" name="date_of_birth" value="<?= htmlspecialchars($editingCustomer['date_of_birth'] ?? '') ?>">
                    </div>
                </div>
                <div class="row g-3 mt-1">
                    <div class="col-12 col-md-6">
                        <label class="form-label">Loại giấy tờ</label>
                        <input type="text" class="form-control" name="id_type" value="<?= htmlspecialchars($editingCustomer['id_type'] ?? '') ?>" placeholder="CCCD, Passport...">
                    </div>
                    <div class="col-12 col-md-6">
                        <label class="form-label">Số giấy tờ</label>
                        <input type="text" class="form-control" name="id_number" value="<?= htmlspecialchars($editingCustomer['id_number'] ?? '') ?>">
                    </div>
                </div>
                <div class="row g-3 mt-1">
                    <div class="col-12 col-md-6">
                        <label class="form-label">Điện thoại</label>
                        <input type="text" class="form-control" name="contact_phone" value="<?= htmlspecialchars($editingCustomer['contact_phone'] ?? '') ?>">
                    </div>
                    <div class="col-12 col-md-6">
                        <label class="form-label">Email</label>
                        <input type="email" class="form-control" name="email" value="<?= htmlspecialchars($editingCustomer['email'] ?? '') ?>">
                    </div>
                </div>
                <div class="mt-3">
                    <label class="form-label">Địa chỉ</label>
                    <input type="text" class="form-control" name="address" value="<?= htmlspecialchars($editingCustomer['address'] ?? '') ?>">
                </div>
                <div class="mt-3">
                    ...
                    <select class="form-select" name="payment_status">
                        ...
                        <?php $currentStatus = $editingCustomer['payment_status'] ?? 'unpaid'; ?>
                ...
                <div class="mt-3">
                    ...
                    <textarea class="form-control" rows="3" name="special_requests"><?= htmlspecialchars($editingCustomer['special_requests'] ?? '') ?></textarea>
                </div>
                <div class="d-flex gap-2 mt-4">
                    <button type="submit" class="btn btn-success">Cập nhật thông tin</button>
                    <a href="<?= BASE_URL ?>?action=customers&booking_id=<?= (int)$selectedBooking['id'] ?>" class="btn btn-light">Hủy</a>
                </div>
            </form>
        </div>
    <?php endif; ?>
